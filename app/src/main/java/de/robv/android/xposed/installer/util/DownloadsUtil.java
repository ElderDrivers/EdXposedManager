...
public class DownloadsUtil {
    ...
    private static DownloadInfo add(Builder b) {
        ...
        String savePath = "/Download/EdXposedInstaller";
        if (b.mModule) {
            savePath += "/modules";
        }
    ...
    private static void showDownloadDialog(final Builder b, final long id) {
        ...
        final MD2Dialog dialog2 = new MD2Dialog(context)
                .title(b.mTitle)
                ...
                while (true) {
                    ...
                    XposedApp.runOnUiThread(() -> {
                        if (info.totalSize <= 0 || info.status != DownloadManager.STATUS_RUNNING) {
                            dialog2.msg(R.string.download_view_waiting);
                        } else {
                            dialog2.msg(R.string.download_running);
                            int downloaded = info.bytesDownloaded / 1024;
                            int total = info.totalSize / 1024;
                            dialog2.updateProgress(downloaded);
                            dialog2.progressMax(total);
                            String progressI = df.format((float) downloaded / total * 100);
                            @SuppressLint("StringFormatMatches")
                            String kb = String.format(context.getString(R.string.download_progress), downloaded, total);
                            dialog2.percent(progressI);
                            dialog2.kbs(kb);
                        }
                    });
                }
